...
float MatchSequence(PasswordSequence *s1, PasswordSequence *s2, MatchParams params) {
  ...
  int s1Events[SYMBOL_TYPE_NUM] = {};
  int s2Events[SYMBOL_TYPE_NUM] = {};
  ...
  if(DEBUG){
    Serial.print("timeScale | timeScaleAbs: ");
    Serial.print(timeScale);
    Serial.print(" ");
    Serial.println(timeScaleAbs);  
  }
  ...
  for (int i = 0; i < s1->list.size(); i++) {
    ...
    float tap1Duration = tap1.start - lastTap1;
    float tap2Duration = tap2.start - lastTap2;
    if (DEBUG){
      Serial.print("tap1Duration | tap2Duration: ");
      Serial.print(tap1Duration);
      Serial.print(" ");
      Serial.println(tap2Duration); 
    }
    ...
    if(DEBUG){
      Serial.print("durationScale | durationScaleAbs: ");
      Serial.print(durationScale);
      Serial.print(" ");
      Serial.println(durationScaleAbs);
    }


    // Check within limits
    if (durationScaleAbs > params.tapDurationScaleMax) {
